...
void setup() {
  ...
  LEDS.addLeds<LED_TYPE, LED_PIN_1, COLOR_ORDER>(leds,
                                                 LED_COUNT * STAIRS_COUNT);
  // LEDS.addLeds<LED_TYPE, LED_PIN_2, COLOR_ORDER>(leds, LED_COUNT *
  // STAIRS_COUNT);
...
void goingDown(int r, int g, int b) {
  for (int i = STAIRS_COUNT - 1; i >= 0; i--) {
    for (int j = LED_COUNT - 1; j >= 0; j--) {
      int currentLedPos = i * LED_COUNT + j;
      for (int x = STAIRS_COUNT * LED_COUNT - 1; x >= currentLedPos; x--) {
        leds[x] = CRGB(r, g, b);
        LEDS.show();
      }
      FastLED.delay(DELAY_LEDS);
...
void loop() {
  // Upstairs Motion
  // if (digitalRead(PIR_PIN_UP_STAIRS) == HIGH) {
